...
class RepoTest(db.Document):
    ...
    def git_status(self, state=None):
        ...
        with app.app_context():
            ...
            data = {
              "state": state,
              "target_url": "http://" + "/".join([s for s in [app.config["SERVER_NAME"], app.config["APPLICATION_ROOT"]]]) + "/repo/{username}/{reponame}/{uuid}".format(username=self.username, reponame=self.reponame, uuid=self.uuid),
